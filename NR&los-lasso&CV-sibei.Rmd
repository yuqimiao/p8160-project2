...
```{r}
newton_direc =function(data,func,start,tol=1e-10,max=200){
  i=0
  cur=start
  stuff=func(data,cur)
  prevloglike=-Inf
...
```
Note: if steo halving is used, then the results are different from Yuqi's
...
lambdas=seq(lambda_max,0,by=-0.5)
len=length(lambdas)
...
beta_1=c(mean(y_matrix),rep(0,10))## lasso dosen't shrink interecpt
...
i=2
for(i in 2:len){
  beta_new=quadratic_appro(beta_new,x_matrix,y_matrix,lambdas[i])
  result4=rbind(result4,c(lambdas[i],beta_new))
  i=i+1
}
result4 %>% as.data.frame()
```
...
for(k in 1:fold_number){
  ...
  lab=c("lambda",str_c("beta",1:11))
  linear_res_fold=glm(y_train~x_train[,-1],family=binomial(link='logit'))
 beta_new=c(mean(y_train),S(coef(linear_res_fold) %>% as.vector(),lambda[1]))
...
result4 %>% as.data.frame() %>% filter(V1==lambdas[order])## the coef corresponds to optimal lambda,note the V1 is the lambda value
```
