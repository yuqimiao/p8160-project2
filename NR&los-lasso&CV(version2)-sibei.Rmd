...
result4=rbind(lab,c(lambdas[1],beta_1))
i=2
beta_vec=beta_1
for(i in 2:len){
...
```{r,include=FALSE}
lambdas2=c(seq(max(abs(beta_total)),0,by=-0.05),0)
...
for(k in 1:fold_number){
  ...
  lab=c("lambda",str_c("beta",1:11))
 beta_vec=c(beta_new[1],S_S(beta_new[2:11],lambdas2[1]))
 result5=rbind(lab,beta_new)
 i=2
 for(i in 2:len2){
   beta_new=quadratic_appro(beta_vec,x_matrix,y_matrix,lambdas2[i])
...
dataframe=tibble(mse=mse_final,
                        lambdas=lambdas2)
p1=ggplot(dataframe,aes(x=lambdas,y=mse))+geom_line()+theme_bw()+labs(title="a")

dataframe1=tibble(mse=mse_final,
                        lambdas=lambdas2) %>% filter(lambdas2<=3)

  p2=ggplot(dataframe1,aes(x=lambdas,y=mse,label=mse))+geom_point(color = ifelse(dataframe1$lambdas ==lambdas2[order],"red", "grey" ))+theme_bw()+labs(title="b")
...
```{r,echo=FALSE}
h=result3[nrow(result3),]
h_h=h[3:13]
names(h_h)=c("intercept",names(data_cancer[,3:12]))
h_h%>%as.data.frame() %>% knitr::kable()
```
...
The range of $\lambda$ we tried is (0,`r max(abs(beta_total))`) , the MSE of each $\lambda$ is in the Figure 1(a). If the range (0,3) is amplified, we can get the (b) plot. The red dot denotes the $\lambda$=`r lambdas2[order]` with smallest MSE `r mins`
