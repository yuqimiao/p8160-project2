...
for(k in 1:fold_number){
  ...
  lab=c("lambda",str_c("beta",1:11))
 beta_vec=c(beta_new[1],S_S(beta_new[2:11],lambdas[1]))
...
dataframe1=tibble(mse=mse_final,
                        lambdas=lambdas) %>% filter(lambdas<=3) %>% 
  mutate(highlight_flag = ifelse(country == 'United States', T, F))

  ggplot(dataframe1,aes(x=lambdas,y=mse,label=mse))+geom_point(color = ifelse(dataframe1$lambdas ==lambdas[order],"red", "grey" ))
