...
# working stuff
 quadratic_coor_wise_once = function(beta_vec,x_matrix,y_matrix,lambda){
  ...
  pre_tar=1/(2*length(y))*(sum(wi*(zi-x%*%beta_vec)^2))+lambda*sum(abs(beta_vec))
  ...
  for(i in 2:11){
    ...
    cur_tar=1/(2*length(y))*(sum(wi*(zi-x%*%beta_update)^2))+lambda*sum(abs(beta_update))
  
 return(c(beta_update,cur_tar,pre_tar))
}

 beta_2=rep(0.02,11)
 quadratic_coor_wise_once(beta_2,x_matrix,y_matrix,lambdas[1])
```
...
## only return the betas that reach the minmun target value for this lambda
 quadratic_coor_wise_all=function(beta_enter,x_matrix,y_matrix,lambda,tol=exp(-10),max=200){
  first=quadratic_coor_wise_once(beta_enter,x_matrix,y_matrix,lambda)
  pre=first[13]
  cur=first[12]
  beta_3=first[1:11]
  iteration=0
  result4=first
  names(result4)=c(str_c("beta",1:11),"cur_tar","pre_tar")
 while((abs(pre-cur)>tol) & iteration < max){ 
   result_new=quadratic_coor_wise_once(beta_3,x_matrix,y_matrix,lambda)
  ...
  c1=dim(result4)[1]
  return(result4[c1,1:11]) }

beta_enter=rep(0.02,11)
quadratic_coor_wise_all(beta_enter,x_matrix,y_matrix,lambdas[100])
