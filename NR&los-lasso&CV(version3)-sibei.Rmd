...
lambdas=seq(0.5, 0.01,length = 100)
len=length(lambdas)
...
# working stuff
 ...
 beta_2=rep(0.02,11)
 quadratic_appro(beta_2,x_matrix,y_matrix,lambdas[98])
```
...
beta_enter=beta_2
for(i in 1:len){
  beta_new=quadratic_appro(beta_enter,x_matrix,y_matrix,lambdas[i])
...
```{r}
lambdas2=seq(3, 0.001,length = 100)
...
for(k in 1:fold_number){
 ...
 beta_enter=beta_2
 for(i in 1:len2){
   beta_new=quadratic_appro(beta_enter,x_train,y_train,lambdas2[i])
