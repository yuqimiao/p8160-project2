...
# working stuff
 quadratic_coor_wise = function(beta_vec,x_matrix,y_matrix,lambda){
  ...
  for(i in 2:11){
    ...
    i=i+1
      ...
      ## once beta=0, the predictor get abandomed, the next lambdas the coef infront of that predictor is 0
  
 return(beta_update)
}

 beta_2=rep(0.02,11)
 quadratic_coor_wise(beta_2,x_matrix,y_matrix,lambdas[98])
```
...
beta_enter=beta_2
for(i in 1:len){
  beta_new=quadratic_coor_wise(beta_enter,x_matrix,y_matrix,lambdas[i])
...
```{r}
lambdas2=seq(14, 0.001,length = 100)
...
for(k in 1:fold_number){
  ...
  lab=c("lambda",str_c("beta",1:11))
 result5=rbind(lab,c("initial",beta_2))
 i=1
 beta_enter=beta_2
 for(i in 1:len2){
   beta_new=quadratic_coor_wise(beta_enter,x_train,y_train,lambdas2[i])
...
# fit optimal lambda with original data
```{r}
lab=c("lambda",str_c("beta",1:11))

result7=rbind(lab,c("initial",beta_2))
i=1
beta_vec2=beta_2
for(i in 1:len2){
  beta_new=quadratic_coor_wise(beta_vec2,x_matrix,y_matrix,lambdas2[i])
...
```
 ...
 then use train function, which can produce the same optimal lambda but different coef and MSE
```{r}
set.seed(2020)
