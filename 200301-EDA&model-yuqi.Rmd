...
tuning_grid = seq(lambda_max, 0, length = 100) ## tuning seq
...
lasso_co_des = function(data, beta_vec,  k = 5, tune_grid,tol = 1e-10, max = 200){
  ...
  for (i in 1:k) {
    ...
    for (lambda in tune_grid) {
      ...
      pre_target = -Inf
      time = 0
      res = c(k = k,lambda = lambda,time = time, cur_target = cur_target, beta_vec)
      while((abs(pre_target-cur_target)>tol) & time < max){
        ...
        beta_pre = beta_vec
         for (j in 1:dim(train)[2]-1) {
           ...
           if (abs(tmp)>lambda) {
             if(tmp > 0) {beta_j = tmp - lambda}
             else {beta_j = tmp + lambda}
           }else{beta_j = 0}
          beta_pre[j] = beta_j
         } 
        ## sparse updating?
        ...
        w_res_vec = sqrt(w_vec)*(z_vec - theta_vec)
        cur_target = (-1/2*n_tr)*t((w_res_vec))%*%(w_res_vec) +lambda*sum(abs(beta_vec[-1]))
        res = c(res,c(k = k,lambda = lambda,time = time, cur_target = cur_target, beta_vec))
      }
    }
  }
  return(res)
...
lasso_co_des(data = cancer_scale, beta_vec = rep(0.02,11), k = 1,tune_grid = tuning_grid,)
```
