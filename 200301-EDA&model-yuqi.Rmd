...
tuning_grid = seq(3, 0.1,length = 100) ## tuning seq
      ...
      while((abs(pre_target-cur_target)>tol) & time < max){
        ...
        beta_pre = beta_vec
         for (j in 1:dim(train)[2]-1) {
           ...
           if (j>=2) {
             if (abs(tmp)>lambda*dim(x_tr)[1]) {
              if(tmp > 0) {tmp = tmp - lambda*dim(x_tr)[1]}
              else {tmp = tmp + lambda*dim(x_tr)[1]}
              }else{
              tmp = 0
...
```{r}
x.mat <- model.matrix(response~., data[-1])[,-1]
y.class <- data$response
